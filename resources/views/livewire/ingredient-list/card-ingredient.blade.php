<div>
    <section class="relative flex w-36 flex-col rounded-lg border bg-white">
        ...
        <div class="rounded-lg border-4 border-transparent pb-10 peer-checked:border-green-500">
            <div class="m-3 mx-auto flex h-16 md:m-7">
                <object class="m-auto max-h-16 w-16 bg-contain" type="image/jpg"
                    data="{{ 'https://spoonacular.com/cdn/ingredients_100x100/' . $ingredient->ingredientAPI->json_response_api['image'] }}">
            ...
            <form class="flex flex-col" wire:submit.prevent="saveIngredient">

                <div class="w-full px-2">
                    <label class="mb-2 block text-xs font-bold uppercase tracking-wide text-gray-700"
                        for="{{ 'grid-weight-' . $ingredient->id }}">
                        waga
                    </label>
                    <input
                        class="block w-full appearance-none rounded border border-gray-200 bg-gray-200 py-1 px-2 leading-tight text-gray-700 focus:border-gray-500 focus:bg-white focus:outline-none"
                        id="{{ 'grid-weight-' . $ingredient->id }}" type="text" value="{{ $ingredient->weight }}"
                        wire:model="weight">
                    @error('weight')
                ...
                <div class="w-full px-2 pt-2">
                    <label class="mb-2 block text-xs font-bold uppercase tracking-wide text-gray-700"
                        for="{{ 'grid-unit-' . $ingredient->id }}">
                        Jednostki wagi
                    </label>
                    <div class="relative">
                        <select
                            class="block w-full appearance-none rounded border border-gray-200 bg-gray-200 py-1 px-2 pr-8 leading-tight text-gray-700 focus:border-gray-500 focus:bg-white focus:outline-none"
                            id="{{ 'grid-unit-' . $ingredient->id }}" wire:model="unit">
                            @foreach ($ingredient->ingredientAPI->json_response_api['possibleUnits'] as $unit)
                                <option value="{{ $unit }}" @if ($unit == $ingredient->unit) selected @endif>
                                    {{ $unit }}</option>
                            @endforeach
                        </select>
                        @error('unit')
                ...
                <button
                    class="my-2 mx-auto w-3/4 rounded border border-blue-500 bg-transparent px-2 py-1 text-xs font-semibold text-blue-700 transition-all hover:border-transparent hover:bg-blue-500 hover:text-white"
