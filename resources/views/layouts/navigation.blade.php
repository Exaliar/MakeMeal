<nav class="border-b border-gray-100 bg-white dark:border-gray-700 dark:bg-gray-800" x-data="{ open: false }">
            ...
            <div class="flex">
                ...
                <div class="flex shrink-0 items-center">
                    <a href="{{ route('category.index') }}">
                        <img class="h-14 rounded-lg" src="{{ asset('storage/logo/logo_288_x_192.jpg') }}" alt="logo">
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-4 sm:flex">
                    <x-nav-link :href="route('edit-category')" :active="session()->has('editCategory')">
                        {{ __('Edit Category') }}
                    </x-nav-link>
                </div>
                <div class="hidden space-x-8 sm:-my-px sm:ml-4 sm:flex">
                    <x-nav-link :href="route('add-category')" :active="session()->has('addCategory')">
                        {{ __('Show/Hide Add Category') }}
                    </x-nav-link>
                </div>
                <div class="hidden space-x-8 sm:-my-px sm:ml-4 sm:flex">
    ...
    <div class="hidden sm:hidden" :class="{ 'block': open, 'hidden': !open }">
        ...
        <div class="space-y-1">
            <x-responsive-nav-link :href="route('edit-category')" :active="session()->has('editCategory')">
                {{ __('Edit Category') }}
            </x-responsive-nav-link>
        </div>
        <div class="space-y-1">
            <x-responsive-nav-link :href="route('add-category')" :active="session()->has('addCategory')">
                {{ __('Show/Hide Add Category') }}
            </x-responsive-nav-link>
        </div>
