                ...
                <ul class="mt-5 flex w-72 flex-col overflow-x-hidden">
                                            ...
                                            @isset($child['children'])
                                                ...
                                                <ul class="flex flex-col peer-checked/collapse:hidden">
                                                    @foreach ($child['children'] as $lastChild)
                                                        <li class="relative">
                                                            @if (isset($edit) && $edit)
                                                                <input class="peer/edit hidden"
                                                                    id="{{ $lastChild['name'] . $lastChild['id'] }}"
                                                                    type="checkbox">
                                                                <label class="absolute right-0 top-0 m-1 h-8 w-8"
                                                                    for="{{ $lastChild['name'] . $lastChild['id'] }}">
                                                                    <svg class="fill-blue-800 transition-colors"
                                                                        viewBox="0 0 32 32" version="1.1"
                                                                        xmlns="http://www.w3.org/2000/svg">
                                                                        <title>Edytuj</title>
                                                                        <path
                                                                ...
                                                                <div
                                                                    ...
                                                                    <form
                                                                        action="{{ route('category.update', $lastChild['id']) }}"
                                                                        method="POST">
                                                                        @csrf
                                                                        @method('PATCH')
                                                                        <input
                                                                            class="absolute top-0 left-0 h-10 w-40 rounded-lg border-2 border-green-500"
                                                                            name="name" type="text"
                                                                            placeholder="{{ $lastChild['name'] }}">
                                                                        <button class="absolute right-10 m-1 h-8 w-8"
                                                                            type="submit">
                                                                            <svg class="fill-green-700 transition-colors hover:fill-green-400"
                                                                                viewBox="0 0 32 32" version="1.1"
                                                                                xmlns="http://www.w3.org/2000/svg">
                                                                    ...
                                                                    <form
                                                                        action="{{ route('category.destroy', $lastChild['id']) }}"
                                                                        method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button class="absolute right-0 m-1 h-8 w-8"
                                                                            type="submit"
                                                                            onclick="return confirm('Czy na pewno chcesz usunąć tą kategorię?')">
                                                                            <svg class="fill-red-700 transition-colors hover:fill-red-400"
                                                                                viewBox="0 0 32 32" version="1.1"
                                                            ...
                                                            <label
                                                                ...
                                                                <svg class="fill-pink-500 transition-colors"
                                                                    viewBox="0 0 91 91" version="1.1"
                                                                    xmlns="http://www.w3.org/2000/svg">
                                                                    <title>right</title>
                                                                    <path class="st0"
                                                                        d="M12.6,28.9c10.2,5.4,19.8,12.2,25.3,22.6c2.8,5.3,10.9,0.6,8-4.7C43,41.5,39,36.9,34.5,33   c13.6-1.6,30-1.2,32.3,14.2c1,6.8,1.3,13.8,1.4,20.7c0.1,6-1.2,12.7,2,18c0.9,1.6,3.6,2.3,4.9,0.6c5.2-6.6,3.7-17.6,3.5-25.7   c-0.2-8.1,0.2-18.2-4.3-25.4c-9-14.5-29.5-13.8-45.3-12c7.6-4.1,14.4-9.5,20.9-15.7c1.1-1.1-0.3-3.1-1.7-2.2   c-10.8,6.9-22.3,12.3-34.6,15.7C10.3,22.2,9.4,27.2,12.6,28.9z" />
                                                ...
                                                <div class="relative w-72">
                                                    ...
                                                    <span
                                                        ...
                                                        <form action="{{ route('category.store') }}" method="POST">
                                                            @csrf
                                                            <input
                                                                class="absolute top-0 left-0 h-9 w-[208px] rounded-lg border-none"
                                                                name="name" type="text"
                            ...
                            @if (isset($add) && $add)
                                <div class="relative w-72">
                                    ...
                                    <span
                                        ...
                                        <form action="{{ route('category.store') }}" method="POST">
                                            @csrf
                                            <input class="absolute top-0 left-0 h-9 w-[228px] rounded-lg border-none"
                    ...
                    @if (isset($add) && $add)
                        <span
                            ...
                            <form action="{{ route('category.store') }}" method="POST">
                                @csrf
                                <input class="absolute top-0 left-0 h-9 w-[248px] rounded-lg border-none"
                                    name="name" type="text" placeholder="Nowa kategoria">
                                <button class="absolute top-0 right-0 h-9 w-9" type="submit">
                                    <svg class="fill-green-700 transition-colors hover:fill-green-400"
                                        viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
