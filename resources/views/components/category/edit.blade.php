...
@if (isset($edit) && $edit)
    <input class="peer/edit hidden" id="{{ $category['name'] . $category['id'] }}" type="checkbox">
    <label class="absolute right-0 top-0 m-1 h-8 w-8" for="{{ $category['name'] . $category['id'] }}">
        <svg class="fill-blue-800 transition-colors" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
            <title>Edytuj</title>
            <path
    ...
    <div class="absolute top-0 -left-80 h-10 w-[248px] rounded-lg bg-gray-100 transition-all peer-checked/edit:left-0">

        <form action="{{ route('category.update', $category['id']) }}" method="POST">
            @csrf
            @method('PATCH')
            <input class="absolute top-0 left-0 h-10 w-40 rounded-lg border-2 border-green-500" name="name"
        ...
        <form action="{{ route('category.destroy', $category['id']) }}" method="POST">
            ...
            <button class="absolute right-0 m-1 h-8 w-8" type="submit"
                ...
                <svg class="fill-red-700 transition-colors hover:fill-red-400" viewBox="0 0 32 32" version="1.1"
                    xmlns="http://www.w3.org/2000/svg">
                    <title>Usuń</title>
                    <path
...
@endif
<label class="absolute left-5 top-0 mt-2 h-5 w-5 rotate-180 transition-all peer-checked/edit:-left-20">
    <svg class="fill-pink-500 transition-colors" viewBox="0 0 91 91" version="1.1" xmlns="http://www.w3.org/2000/svg">
        <title>right</title>
        <path class="st0"
...
<label class="absolute -left-1 top-0 mt-2 h-5 w-5 transition-all peer-checked/edit:-left-20">
    <svg class="fill-blue-800 transition-colors" viewBox="0 0 91 91" version="1.1" xmlns="http://www.w3.org/2000/svg">
        <title>down</title>
        <path class="st0"
